...
const CustomProjectCard: React.FC<CustomProjectCardProps> = ({ onCustomProject }) => (
    <div className="lg:col-span-2 bg-gradient-to-br from-neutral-900 to-neutral-800 rounded-3xl p-10 border border-neutral-800 flex flex-col md:flex-row items-center gap-10">
        <div className="flex-1">
            <div className="inline-block bg-white text-black px-3 py-1 rounded-md text-xs font-bold uppercase mb-4">
                Enterprise
            </div>
            ...
            <div className="flex flex-wrap gap-3 mb-8">
                {['Custom Web Apps', 'SaaS Development', 'Complex APIs', 'Multi-language'].map((tag, i) => (
                    <span key={i} className="px-3 py-1 bg-neutral-950 rounded-full text-xs text-neutral-400 border border-neutral-800">
                        {tag}
                    </span>
                ))}
            </div>
            <button
                ...
                className="bg-white text-black px-8 py-3 rounded-xl font-bold hover:bg-neutral-200 transition-colors"
            >
